...
import { ConsoleLogUtils, ConsoleColour as ConsoleTextEffect, ConsoleColour } from "./Utils/ConsoleLogUtils";
...
export class App {
	...
	constructor() {
		...
		this.App = express();
		let server = new InversifyExpressServer(container, null, null, this.App);
	...
	private CheckEnvironmentVariables() {
		const { MONGO_USER, MONGO_PASSWORD, MONGO_PATH, ORIGIN_ADDRESS, PORT, APP_SECRET } = process.env;
		...
		if (errorLines) {
			ConsoleLogUtils.LogToConsole("--------------------------------------------------", ConsoleTextEffect.FgRed);
			ConsoleLogUtils.LogToConsole("Not all environment variables are set up properly", ConsoleTextEffect.FgRed);
			console.log(errorLines);
			console.log("\nTo fix this: Create a .env file in the server root");
			console.log("and add the above mentioned variables to it");
			ConsoleLogUtils.LogToConsole("--------------------------------------------------", ConsoleTextEffect.FgRed);
		} else {
			ConsoleLogUtils.LogToConsole("All environment variables present and accounted for", ConsoleTextEffect.FgGreen);
		}
	}
	...
	private InitializeMiddleware() {
		...
		this.App.use((req, res: any, next) => {
			let extensionManager = new ExtensionManager(new ResponseExtensions(res));
